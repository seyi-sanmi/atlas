  ...
  return (
    <Sheet>
      ...
      <SheetTrigger asChild>
        <div
          onClick={handleClick}
           className={`group relative border transition-all duration-300 cursor-pointer overflow-hidden bg-secondary-bg  
    ${viewMode === "grid"
      ? "flex flex-col h-[350px] sm-[400px] rounded-lg  border border-primary-border"
      : "flex flex-col sm:flex-row min-h-[180px] rounded-lg rounded-tl-none border border-primary-border"
    }
  `}
                    tabIndex={0}
          role="button"
    ...
    ${viewMode === "grid" ? "h-40 sm:h-55 w-full" : "w-full h-50 sm:w-1/3 sm:h-auto"}
  `}>
            {/* Starred indicator */}
            {event.is_starred && (
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div   className={`font-bold text-white font-display tracking-tight ${
    viewMode === "grid" ? "text-3xl sm:text-3xl" : "text-4xl sm:text-4xl"
  }`}>
                      {eventDate.getDate().toString().padStart(2, "0")}
                    </div>
                   <div className={`font-medium uppercase tracking-wider text-white/80 ${
  viewMode === "grid" ? "text-sm sm:text-lg" : "text-lg sm:text-xl"
}`}>
  {eventDate.toLocaleDateString("en-US", { month: "short" })}
</div>

<div className={`font-medium text-white/60 ${
  viewMode === "grid" ? "text-xs sm:text-base" : "text-base sm:text-lg"
}`}>
  {eventDate.getFullYear()}
    ...
    ${viewMode === "grid" ? "p-3 pt-2 space-y-1" : "p-4 pt-2 sm:pt-4 space-y-2.5"}
  `}>
   {viewMode === "grid" ? (
  <>
      {/* Title */}
 ...
 <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4   min-h-[.5rem] sm:min-h-[1.5rem]  text-[10px] text-primary-text/60">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {/* Research Area Tags */}
              <div className="relative area_research_tag   min-h-[2rem] ">
                        ...
                        {event.ai_interest_areas.map((area) => {
                          ...
                          return (
                            <button
                              ...
                              className={`inline-block px-2 py-1 text-[10px] font-medium rounded-full border transition-all duration-200 cursor-pointer ${
                                isSelected
                  ...
                  <div className="w-full h-48 object-cover rounded-lg shadow-lg mb-2">
                    <div className="rounded-lg overflow-hidden h-full">
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="text-center">
                            <div className="text-2xl sm:text-2xl font-bold text-white font-display tracking-tight">
                              {eventDate.getDate()}
                            </div>
