...
// Search events by title, description, location, organizer, or categories
export async function searchEvents(query: string): Promise<Event[]> {
  ...
  const { data, error } = await supabase
    .from('events')
    .select('*')
    .or(`title.ilike.%${query}%, description.ilike.%${query}%, location.ilike.%${query}%, organizer.ilike.%${query}%`)
    .order('date', { ascending: true })
...
export async function searchAndFilterEvents(options: {
  ...
  if (options.query && options.query.trim()) {
    queryBuilder = queryBuilder.or(`title.ilike.%${options.query}%, description.ilike.%${options.query}%, location.ilike.%${options.query}%, organizer.ilike.%${options.query}%`)
  }
...
export async function getUniqueLocations(): Promise<string[]> {
  ...
  // Extract unique locations and filter out empty/null values
